...
export const ShipmentsPage = () => {
  ...
  useEffect(() => {
    if (token) {
      void Promise.all([listShipmentsRequest(token, terminal), listBlendsRequest(token)]).then(([shipments, availableBlends]) => {
        setData(shipments);
        setBlends(availableBlends.filter((item) => item.isActive));
      });
    }
  }, [token, terminal]);
