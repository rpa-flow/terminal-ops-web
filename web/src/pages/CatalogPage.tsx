  ...
  return <main className="min-h-screen bg-surface">
    ...
    <section className="mx-auto grid max-w-7xl gap-4 px-4 py-6 md:grid-cols-2">
      <section className="rounded border border-outline-variant bg-surface-container-lowest p-4 shadow-sm">
        ...
        <ul className="mt-4 space-y-2">{materials.map(m=><li key={m.id} className="flex items-center justify-between rounded border border-outline-variant p-2"><span>{m.name}</span><button className="btn-muted" onClick={()=>token&&deactivateMaterialRequest(token,m.id).then(load)}>Inativar</button></li>)}</ul>
      </section>
      <section className="rounded border border-outline-variant bg-surface-container-lowest p-4 shadow-sm">
        ...
        <ul className="mt-4 space-y-2">{suppliers.map(s=><li key={s.id} className="flex items-center justify-between rounded border border-outline-variant p-2"><span>{s.name}</span><button className="btn-muted" onClick={()=>token&&deactivateSupplierRequest(token,s.id).then(load)}>Inativar</button></li>)}</ul>
      </section>
    </section>
  </main>
}
